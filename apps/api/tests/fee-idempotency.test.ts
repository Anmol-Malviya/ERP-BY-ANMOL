...
describe('fee ledger idempotency',()=>{
 ...
 it('rejects reuse of a key with different money',async()=>{await runWithTenant({schoolId},async()=>{const inv=await invoice();await collectManualPayment({invoiceId:inv._id,amount:200,mode:'CASH',idempotencyKey:'cash_payment_002',collectedBy:null});await expect(collectManualPayment({invoiceId:inv._id,amount:300,mode:'CASH',idempotencyKey:'cash_payment_002',collectedBy:null})).rejects.toMatchObject({statusCode:409})})});
 it('prevents cumulative refund requests above the payment amount',async()=>{await runWithTenant({schoolId},async()=>{const inv=await invoice();const result=await collectManualPayment({invoiceId:inv._id,amount:500,mode:'CASH',idempotencyKey:'cash_payment_003',collectedBy:null});expect(result.payment).not.toBeNull();await requestRefund({paymentId:result.payment!._id,amount:300,idempotencyKey:'refund_req_001',requestedBy:null});await expect(requestRefund({paymentId:result.payment!._id,amount:250,idempotencyKey:'refund_req_002',requestedBy:null})).rejects.toMatchObject({statusCode:409})})});
});
