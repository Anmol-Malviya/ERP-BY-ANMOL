import { DEFAULT_ROLE_PERMISSIONS,PERMISSIONS,type Permission,type SchoolRole } from '@erp/contracts';
...
export function effectivePermissions(request:any){const explicit=request.auth?.permissions;if(Array.isArray(explicit))return new Set<Permission>(explicit as Permission[]);const role=request.auth?.role as SchoolRole|undefined;return new Set<Permission>(role?(DEFAULT_ROLE_PERMISSIONS[role]??[]):[])}
