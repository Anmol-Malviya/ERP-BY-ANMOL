...
export type RazorpayOrder={id:string;amount:number;amount_due:number;currency:string;receipt:string;status:string};
export type RazorpayPayment={id:string;amount:number;currency:string;status:'created'|'authorized'|'captured'|'refunded'|'failed'|string;captured:boolean;order_id:string|null;error_description?:string|null};
export type RazorpayRefund={id:string;amount:number;payment_id:string;status:'pending'|'processed'|'failed'|string};
type RazorpayPaymentCollection={entity:string;count:number;items:RazorpayPayment[]};
...
export async function fetchOrderPayments(orderId:string){return api<RazorpayPaymentCollection>(`/orders/${encodeURIComponent(orderId)}/payments`)}
...
export async function fetchRefund(refundId:string){return api<RazorpayRefund>(`/refunds/${encodeURIComponent(refundId)}`)}
...
export function verifyWebhookSignature(rawBody:Buffer,signature:string){if(!env.RAZORPAY_WEBHOOK_SECRET)throw new AppError(503,'WEBHOOK_NOT_CONFIGURED','Razorpay webhook secret is not configured');const expected=createHmac('sha256',env.RAZORPAY_WEBHOOK_SECRET).update(rawBody).digest('hex');if(!/^[a-f0-9]{64}$/i.test(signature)||signature.length!==expected.length)return false;return timingSafeEqual(Buffer.from(expected,'hex'),Buffer.from(signature,'hex'))}
export function toPaise(amount:number){const value=Number(amount),paise=Math.round(value*100);if(!Number.isFinite(value)||value<=0||Math.abs(value*100-paise)>0.000001)throw new AppError(400,'INVALID_AMOUNT','Amount must be positive with at most two decimal places');return paise}
