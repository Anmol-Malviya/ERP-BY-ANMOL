...
export function tenantPlugin(schema:Schema){
  ...
  for(const hook of QUERY_HOOKS){
    schema.pre(hook as any,function(this:any,next:any){
      ...
      if(!ctx?.isPlatform){
        if(!ctx?.schoolId)return next(new Error('Tenant context missing'));
  ...
  schema.pre('aggregate',function(this:any,next:any){
    ...
    if(!ctx?.isPlatform){
      if(!ctx?.schoolId)return next(new Error('Tenant context missing'));
      this.pipeline().unshift({$match:{schoolId:new Types.ObjectId(ctx.schoolId),deletedAt:null}});
    }
    next();
  });
  ...
  schema.pre('validate',function(this:any,next:any){
    ...
    if(!ctx?.isPlatform){
      if(!ctx?.schoolId)return next(new Error('Tenant context missing'));
      if(!this.schoolId)this.schoolId=ctx.schoolId;
      if(String(this.schoolId)!==String(ctx.schoolId))return next(new Error('Cross-tenant write rejected'));
    }
    next();
  });
}
