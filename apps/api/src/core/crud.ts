...
type DataResolver=(request:any)=>Record<string,unknown>|Promise<Record<string,unknown>>;
type CrudOptions={
  prefix:string;model:Model<any>;entity:string;readPermission:Permission;writePermission:Permission;searchable?:string[];
  readScope?:ScopeResolver;writeScope?:ScopeResolver;readProjection?:ProjectionResolver;
  prepareCreate?:DataResolver;prepareUpdate?:DataResolver;
};
...
export async function registerCrud(app:FastifyInstance,o:CrudOptions){
 ...
 app.post(prefix,{preHandler:requirePermission(writePermission)},async(request:any)=>{const data=o.prepareCreate?await o.prepareCreate(request):request.body;const item=await model.create(data);await writeAudit({action:'create',entity,entityId:String(item._id),after:item.toObject(),ip:request.ip});return{success:true,data:item}});
 app.patch(`${prefix}/:id`,{preHandler:requirePermission(writePermission)},async(request:any)=>{const scope=await o.writeScope?.(request)??{},filter=scoped(scope,{_id:request.params.id});const before=await model.findOne(filter).lean();if(!before)throw notFound(`${entity} not found`);const data=o.prepareUpdate?await o.prepareUpdate(request):request.body;const item=await model.findOneAndUpdate(filter,{$set:data},{new:true,runValidators:true});await writeAudit({action:'update',entity,entityId:request.params.id,before,after:item?.toObject(),ip:request.ip});return{success:true,data:item}});
