...
export type ReportJobPayload={reportCardId:string;schoolId:string;bucket:string;key:string;fileName:string;render:{schoolName:string;schoolAddress?:string;studentName:string;admissionNo:string;rollNo?:string;className?:string;sectionName?:string;examName:string;term?:string;subjects:Array<{name:string;maxMarks:number;obtained:number;grade?:string;remark?:string}>;obtained:number;maximum:number;percentage:number;grade:string;result:string;attendancePercentage:number;attendancePresent:number;attendanceTotal:number;published:boolean}};

let redis:Redis|undefined;
let emailQueue:Queue<EmailJob>|undefined,fileScanQueue:Queue<FileScanJob>|undefined,paymentEventQueue:Queue<PaymentEventJob>|undefined,paymentReconcileQueue:Queue<Record<string,never>>|undefined,pdfQueue:Queue<PdfJob>|undefined,importQueue:Queue<ImportJobPayload>|undefined,reportQueue:Queue<ReportJobPayload>|undefined;
...
function reports(){return reportQueue??=new Queue<ReportJobPayload>('erp:report',{connection:connection(),defaultJobOptions:{attempts:4,backoff:{type:'exponential',delay:3_000},removeOnComplete:1_000,removeOnFail:2_000}})}
...
export async function enqueueReport(data:ReportJobPayload){return reports().add('render',data,{jobId:`report-card-${data.reportCardId}`})}
...
export async function closeQueues(){await Promise.all([emailQueue?.close(),fileScanQueue?.close(),paymentEventQueue?.close(),paymentReconcileQueue?.close(),pdfQueue?.close(),importQueue?.close(),reportQueue?.close()].filter(Boolean) as Promise<unknown>[]);if(redis&&redis.status!=='end')await redis.quit().catch(()=>undefined);emailQueue=undefined;fileScanQueue=undefined;paymentEventQueue=undefined;paymentReconcileQueue=undefined;pdfQueue=undefined;importQueue=undefined;reportQueue=undefined;redis=undefined}
