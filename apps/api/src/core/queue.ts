...
let emailQueue:Queue<EmailJob>|undefined,fileScanQueue:Queue<FileScanJob>|undefined,paymentEventQueue:Queue<PaymentEventJob>|undefined,paymentReconcileQueue:Queue<Record<string,never>>|undefined,pdfQueue:Queue<PdfJob>|undefined;
...
export async function closeQueues(){await Promise.all([emailQueue?.close(),fileScanQueue?.close(),paymentEventQueue?.close(),paymentReconcileQueue?.close(),pdfQueue?.close()].filter(Boolean) as Promise<unknown>[]);if(redis&&redis.status!=='end')await redis.quit().catch(()=>undefined);emailQueue=undefined;fileScanQueue=undefined;paymentEventQueue=undefined;paymentReconcileQueue=undefined;pdfQueue=undefined;redis=undefined}
